	  ...
	  this.fullScreenToggle = function (e) {
	    ...
	    if (
	      ...
	      if (this.getFullScreenElement()) {
	        ...
	        else if (document.webkitExitFullscreen) { document.webkitExitFullscreen() } else if (document.mozCancelFullScreen) document.mozCancelFullScreen()
		      ...
		      if (
		        ...
		        if (dsField.constructor == Number) {
		          ...
		          if (
		            ...
		            else { fieldValue = CIQ.displayableDate(stx, panel.chart, dsField) }
		          } else {
		        ...
		        if (dedicatedField) {
		          ...
		          if (fieldNameField.innerHTML === '') {
		            ...
		            if (CIQ.I18N && CIQ.I18N.localized) { CIQ.I18N.translateUI(null, fieldNameField) }
		          }
		        } else {
	...
	function (config) {
	  ...
	  stx.prepend('updateChartData', function (appendQuotes, chart, params) {
	    ...
	    function completeLastBar (record) {
	      ...
	      for (var md = chart.masterData.length - 1; md >= 0; md--) {
	        ...
	        if (bar.Close || bar.Close === 0) {
	          ...
	          if (record.Open === undefined && bar.Open !== undefined) { bar.Open = record.Close }
	          if (record.High === undefined && bar.High !== undefined) { bar.High = record.Close }
	          if (record.Low === undefined && bar.Low !== undefined) { bar.Low = record.Close }
	    ...
	    if (supportedChartType) {
	      ...
	      if (interval == 'day' || timeUnit == 'day') { chartJustAdvanced = quote.DT.getDate() != this.prevQuote.DT.getDate() } else if (interval == 'week' || timeUnit == 'week') {
	        chartJustAdvanced =
  ...
  CIQ.Outliers =
	...
	function (params) {
	  ...
	  this.processDataSet = function (dataSet, panel, axis) {
	    ...
	    ) { result[0] = currentAxis.outlierMin } else result[0] = currentAxis.trendMin
	    ...
	    ) { result[1] = currentAxis.outlierMax } else result[1] = currentAxis.trendMax
	  ...
	  this.find = function (dataSet) {
	    ...
	    for (var idx = 0; idx < dataLength; idx++) {
	      ...
	      if (dataHigh.value >= maxValue) { activeOutliers.push(createMarkerPlaceholder(dataHigh, 'high')) }

	      // Find the first low value that's less than or equal to outlier threshold min
	      if (outlierMin === null && dataLow.value <= minValue) { outlierMin = dataLow.value }
	      // Find the first high value that's greater than or equal to outlier threshold max
	      if (outlierMax === null && dataHigh.value >= maxValue) { outlierMax = dataHigh.value }

	      // Find the first low value that's greater than the outlier threshold min
	      if (trendMin === null && dataLow.value > minValue) { trendMin = dataLow.value }
	      // Find the first high value that's less than the outlier threshold max
	      if (trendMax === null && dataHigh.value < maxValue) { trendMax = dataHigh.value }
	  ...
	  this.updateMarkerVisibility = function () {
	    Object.keys(this.markers).forEach(
	      function (key) {
	        ...
	        ) { this.markers[key].marker.node.style.display = 'none' } else this.markers[key].marker.node.style.display = 'block'
	      }.bind(this)
	    )
	  }
