...
import {CIQ} from './componentUI.js'
  ...
  if (!CIQ.Marker) {
    ...
    class Advertisement extends CIQ.UI.ModalTag {
      ...
      setNameValueStore (nameValueStore) {
        if (!nameValueStore && CIQ.NameValueStore) { nameValueStore = CIQ.NameValueStore }
      ...
      displayAttribution (stx) {
        ...
        if (chart.attribution) {
          ...
          if (source + exchange != chartAttrib.attr('lastAttrib')) {
            ...
            if (CIQ.I18N && CIQ.I18N.localized) { CIQ.I18N.translateUI(null, chartAttrib[0]) }
        ...
        for (var study in layout.studies) {
          ...
          if (this.messages.sources[type]) {
            ...
            if (CIQ.I18N && CIQ.I18N.localized) { CIQ.I18N.translateUI(null, attrib[0]) }
    ...
    update () {
      ...
      if (
        ...
        if (todaysChangePctDiv) {
          if (todaysChangePctDiv.innerText !== todaysChangeDisplay) { todaysChangePctDiv.innerText = todaysChangeDisplay }
        }
    ...
    startPriceTracker (updatePrices) {
      ...
      this.addInjection('append', 'createDataSet', function () {
        ...
        if (this.chart.dataSet && this.chart.dataSet.length) {
          if (self.node.attr('cq-show') !== 'true') { self.node.attr('cq-show', 'true') }
  ...
  class DataDialog extends CIQ.UI.DialogContentTag {
    ...
    abortImport (e) {
      ...
      for (let fset of this.form.querySelectorAll('fieldset[name]')) {
        ...
        if (e && e.target.getAttribute('fields') !== fname && !targetClose) { continue }
 ...
 */
  class Dialog extends CIQ.UI.BaseComponent {
    ...
    onKeyboardDeselection () {
      ...
      if (this.keyboardNavigation && this.keyboardNavigation !== null) { this.keyboardNavigation.highlightAlign() }
    }
    ...
    processEsc (hub) {
      ...
      if (this.closeActiveMenu()) {
        ...
        if (hub.tabActiveModals[0]) { hub.setKeyControlElement(hub.tabActiveModals[0]) }
    ...
    open (params) {
      ...
      if (CIQ.I18N && CIQ.I18N.localized) { CIQ.I18N.translateUI(null, this.node[0]) }
    ...
    show (params) {
      ...
      setTimeout(function () {
        ...
        if (self.uiManager.overlay && !params.bypassOverlay) {
          if (self.uiManager.overlay.getAttribute('cq-active') !== 'true') { self.uiManager.overlay.setAttribute('cq-active', 'true') }
        }
        ...
        for (var attribute in self.activeAttributes) {
          if (self.node.attr(attribute) !== 'true') { self.node.attr(attribute, 'true') }
        }
        self.resize()
      })
 ...
 */
  class FloatingWindow extends CIQ.UI.ContextTag {
    ...
    onDocumentation ({
      ...
      if (CIQ.I18N && CIQ.I18N.localized) { CIQ.I18N.translateUI(null, this.documentationWindow.w) }
 ...
 */
  class Help extends CIQ.UI.ContextTag {
    ...
    setContext () {
      ...
      if (CIQ.Help && !topNode.hasAttribute(attributeName)) { topNode.setAttribute(attributeName, '') }
    ...
    press () {
      ...
      if (!helpData.actions) { helpData.actions = [CIQ.Help.Actions.close, CIQ.Help.Actions.enable] }
 ...
 */
  class InfoToggle extends CIQ.UI.ContextTag {
    ...
    setTooltip (value) {
      ...
      tooltip.innerText = stx.translateIf(`Info ${value || 'off'}`)
    }
    ...
    class LanguageDialog extends CIQ.UI.DialogContentTag {
      ...
      open (params) {
        ...
        if (document.body.keystrokeHub) {
          ...
          if (tabActiveModals[0]) { document.body.keystrokeHub.setKeyControlElement(tabActiveModals[0]) }
        }
      }
    }
  ...
  if (!CIQ.ChartEngine.Driver) {
    ...
    class Lookup extends CIQ.UI.ContextTag {
        ...
        switch (key) {
          ...
          case 'Right':
            if (focusedElements.length) {
              ...
              if (this.resultList[0].keyboardNavigation) { this.resultList[0].keyboardNavigation = null }
          ...
          case 'Down':
            ...
            if (!focusedElements.length) {
              ...
              if (resultItem) { this.resultList[0].keyboardNavigation = this.keyboardNavigation }
            }
            CIQ.blur(input)
            break
          case 'ArrowUp':
      ...
      setContext (context) {
        ...
        if (!this.params.cb && context.changeSymbol) { this.params.cb = context.changeSymbol }
      }

      changeContext (newContext) {
        if (this.params.cb && newContext.changeSymbol) { this.params.cb = newContext.changeSymbol }
      }
 ...
 */
  class Menu extends HTMLElement {
    ...
    show (params) {
      ...
      if (this.keyboardNavigation) {
        const dropdown = this.querySelector('cq-menu-dropdown')
        if (dropdown) { this.keyboardNavigation.setKeyControlElement(dropdown, true) }
      }
    }
  ...
  class MessageToaster extends CIQ.UI.ContextTag {
    ...
    handleResize () {
      for (let idx = 0; idx < this.messageQueue.length; idx++) {
        if (this.messageQueue[idx].isDisplayed) { this.positionElement(this.messageQueue[idx]) }
      }
    }
    ...
    createMessageElement (notification) {
      ...
      if (transition) { messageElement.classList.add('animate', 'transition-' + transition) }
    ...
    removeMessageNode (messageNode) {
      ...
      window.setTimeout(() => {
        ...
        if (this.displayTimer) { this.displayTimer = window.clearTimeout(this.displayTimer) }
    ...
    newMessage (notification) {
      if (typeof notification === 'string' && this.context.config) {
        if (this.context.config.systemMessages) { notification = this.context.config.systemMessages[notification] }
      }
      ...
      if (newMessage) {
        ...
        if (index >= 0) {
          ...
          if (this.messageQueue[index].isDisplayed) { this.recallMessageNode(this.messageQueue[index]) }
 ...
 */
  class Scroll extends CIQ.UI.BaseComponent {
    ...
    onKeyboardDeselection () {
      // If we're using keyboard navigation, return the highlight to the tab selected element
      if (this.keyboardNavigation && this.keyboardNavigation !== null) { this.keyboardNavigation.highlightPosition() }
    }
  ...
  class SidePanel extends CIQ.UI.ContextTag {
    ...
    close () {
      ...
      children.each(function () {
        if (this.sidePanelActiveClass) { this.classList.remove(this.sidePanelActiveClass) }
  ...
  if (!CIQ.Studies) {
    ...
    class StudyDialog extends CIQ.UI.DialogContentTag {
      ...
      open (params) {
        ...
        if (typeof params.addWhenDone !== 'undefined') { this.addWhenDone = params.addWhenDone }
      ...
      refreshInputs (empty) {
        ...
        for (var i = 0; i < this.helper.inputs.length; i++) {
          ...
          if (input.type == 'number') {
            ...
            if (input.type == 'date') { formField.value = this.formatDateInput(input.value) } else if (input.type == 'time') { formField.value = this.formatTimeInput(input.value) } else formField.value = input.value
            this.setChangeEvent(formField, 'inputs', input.name)
            for (iAttr in attributes) {
              if (iAttr !== 'hidden') { formField.setAttribute(iAttr, attributes[iAttr]) }
            }
            ...
            if (attributes && attributes.readonly) { formField.setAttribute('readonly', attributes.readonly) }
        ...
        for (var i = 0; i < this.helper.parameters.length; i++) {
          ...
          if (parameter.defaultValue.constructor == Boolean) {
            ...
            for (pAttr in attributes) {
              if (pAttr !== 'hidden') { paramInput.setAttribute(pAttr, attributes[pAttr]) }
            }
    ...
    class StudyLegend extends CIQ.UI.ModalTag {
      ...
      setActiveState (newState) {
        ...
        if (newState) {
          ...
          // Treat the legend like a modal so keyboard navigation is returned after using colorPicker
          if (keystrokeHub && this.keyboardNavigation) { keystrokeHub.addActiveModal(this) }
        } else {
 ...
 */
  class Themes extends CIQ.UI.ContextTag {
    ...
    configureMenu () {
      ...
      if (CIQ.I18N && CIQ.I18N.localized) { CIQ.I18N.translateUI(null, this.builtInMenu[0]) }
    ...
    initialize (params) {
      ...
      if (nameValueStore) {
        if (isConstructor(nameValueStore)) { nameValueStore = this.params.nameValueStore = new nameValueStore() }
 ...
 */
  class Toggle extends CIQ.UI.ContextTag {
    ...
    disconnectObservable () {
      ...
      if (member && obj && listener) { CIQ.UI.unobserveProperty(member, obj, listener) }
    }
    ...
    setContext ({ config }) {
      ...
      for (var i = 0; i < this.params.toggles.length; i++) {
        ...
        else if (toggle === 'true' || toggle === 'false') { toggles[i] = toggle === 'true' } else if (!isNaN(Number(toggle))) toggles[i] = Number(toggle)
      }
    ...
    class DrawingToolbar extends CIQ.UI.ContextTag {
      ...
      defaultElements (drawingParameters) {
        ...
        for (var param in drawingParameters) {
          ...
          else if (param == 'pattern' || param == 'lineWidth') { arr.push('cq-line-style') } else if (param == 'axisLabel') arr.push('cq-axis-label')
      ...
      setFontSize (activator, fontSize) {
        ...
        if (stx.currentVectorParameters.annotation) { stx.currentVectorParameters.annotation.font.size = fontSize }
    ...
    save () {
      ...
      document.querySelectorAll('cq-views').forEach(function (v) {
        ...
        if (!madeChange) {
          ...
          madeChange = true
          if (v.params.nameValueStore) { v.params.nameValueStore.set('stx-views', obj.views) }
        }
      })
      this.close()
    }
  ...
  if (!Palette) {
    ...
    class DrawingSettings extends Palette.classDefinition {
      ...
      setContext (context) {
        ...
        this.sync()
        this.dirty(false)
        if (context.stx.setDrawingContainer) { context.stx.setDrawingContainer(this) }
        var self = this
        context.stx.addEventListener('theme', function (obj) {
          var isDirty = self.node
            .find('*[cq-toolbar-dirty]')
