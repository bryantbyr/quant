...
class BaseComponent extends HTMLElement {
  ...
  clickItem (item, e, originationElement) {
    ...
    if (item.selectFC) {
      ...
      if (originationElement) { item.keyboardOriginationElement = originationElement }
      // Simulate a click
...
CIQ.UI.Helper.prototype.changeContext = function (newContext) {
  ...
  if (this.implementation && this.implementation.stx) { this.implementation.stx = newContext.stx }
...
DrawingEdit.prototype.text = function () {
  ...
  if (this.drawingContext) {
    ...
    if (this.drawingContext.drawing.edit) { this.drawingContext.drawing.edit(null, true) }
  }
}
/**
    ...
    case 'escape':
      ...
      if (CIQ.ChartEngine.drawingLine) {
        ...
        if (keystrokeHub.tabActiveModals.length) {
          ...
          if (keystrokeHub.tabActiveModals[0]) { keystrokeHub.setKeyControlElement(keystrokeHub.tabActiveModals[0]) }
        } else {
...
CIQ.UI.KeystrokeHub.prototype.sortTabOrderElements = function (
  ...
  if (!elementToSelect && this.tabIndex !== 0) {
    // An item is selected. Hold on to it in case its position in the array changes.
    if (this.tabOrder[Math.abs(this.tabIndex)]) { elementToSelect = this.tabOrder[Math.abs(this.tabIndex)].element }
  }
...
CIQ.UI.KeystrokeHub.prototype.setKeyControlElement = function (
  ...
  if (element) {
    ...
    if (typeof this.keyControlElement.onKeyboardSelection === 'function') { this.keyControlElement.onKeyboardSelection() }
  }
}
/**
...
CIQ.UI.KeystrokeHub.prototype.tabOrderDeselect = function (eventsOnly) {
  ...
  if (tabOrder[tabIndex]) { this.disableKeyControlElement(tabOrder[tabIndex].element) }
  if (!eventsOnly) {
    this.tabActiveElement = null
  ...
  breakpointSetter (uiContext) {
    return function setBreakpointClass (value) {
      ...
      breakpoints.forEach((breakpointGroup) => {
        if (breakpointGroup.includes(value)) { contextClasses.remove(...breakpointGroup) }
      })
...
class Faquery {
  constructor (input, invalid) {
    if (invalid !== undefined) { console.log('CIQ.UI.$() only accepts one input parameter.') }
    let elements = []
    ...
    else if (typeof input === 'string') { elements = elements = Array.from(document.querySelectorAll(input)) }
  ...
  append (content, invalid) {
    if (typeof invalid !== 'undefined') { console.log('CIQ.UI.$#append only accepts one argument.') }
