    ...
    for (let i = start; i < end; i += dev) {
      ...
      if (date > 14 && date < 22) {
        ...
        if (month > 0) {
          ...
          if (expTime - todayTime > 65 * ONE_DAY && cycle != (month + 1) % 3) { continue } // out of cycle
        }
...
const generateRandomInstrumentValues = (symbol, type, basis, factor) => {
  ...
  for (let i = 0; i < myExpirations.length; i++) {
    ...
    if (parts && parts.length > 5) {
      ...
      // volatility?
      if (parts[4] === 'C') { price += (0.1 * (strike - basis / 2) * strike) / basis } else price += 0.1 * ((3 / 2) * basis - strike) * (2 - strike / basis)
...
const getFractionOfInterval = ({ interval, period }, date, isHistorical) => {
  ...
  if (elapsedOfDay === SECONDS_IN_TRADING_DAY) { elapsedOfDay = elapsedOfDay - 0.1 }
  if (interval === 'month') {
    return (
  ...
  if (interval === 'week') { return (date.getDay() - 1 + elapsedOfDay) / (5 * SECONDS_IN_TRADING_DAY) }
  if (interval === 'day') return elapsedOfDay / SECONDS_IN_TRADING_DAY
  if (interval === 'minute') { return (elapsedOfDay % (60 * period)) / SECONDS_IN_TRADING_DAY }
  if (interval === 'second') { return (elapsedOfDay % period) / SECONDS_IN_TRADING_DAY }
  return 0
}
const logTables = (newQuote) => {
  ...
  for (let i in newQuote.optionChain) {
    ...
    if (!volByStrike[option.strike.value]) { volByStrike[option.strike.value] = { callvolume: 0, putvolume: 0 } }
...
optionChainSimulator.fetchInitialData = function (
  ...
  function callback (obj) {
    ...
    for (
      ...
      if (daily || !isMarketClosed(recentQuote.DT)) { quotesToPopulate.push(recentQuote) }
    }
